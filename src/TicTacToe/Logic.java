...
public class Logic {
    ...
    static String WinToHuman = "Крестики Выиграли!!!";
    static String WinToIA = "Нолики Выиграли!!!";
    static String NoWin = "Ничья";
    ...
    public static void go2() {
        ...
        humanTurn(BattleMap.cellX2,BattleMap.cellY2);
        printMap();
        if (checkWinLines(DOT_O, DOTS_TO_WIN)) {
            System.out.println(WinToIA);
            return;
    ...
    static void humanTurn2(int x, int y){
        if (isCellValid(y, x)) {
            map[y][x] = DOT_O;
            go2();
...
// Сходить в произвольную не занятую ячейку
    ...
    static boolean checkLine(int cy, int cx, int vy, int vx, char dot, int dotsToWin) {
        ...
        pointX1 =cy;
        pointY1 =cx;
        pointX2 =cy +vy * (dotsToWin - 1);
        pointY2 =cx +vx * (dotsToWin - 1);
